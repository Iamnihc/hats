...
class hatsGame {
  ...
  public getGameInfo() {
    return {
      playing: this.currentPlayer.getSimple(),
  ...
  public checkWord(word) {
    return strip(word) == strip(this.currentWord);
  }
}
...
function getPrettyUsers(roomNum) {
  ...
  return hatGames[roomNum].users.map((user) => [
    user.nickname,
    user.isReady,
    user.guessed,
    user.online,
...
io.on("connection", function (socket) {
  ...
  socket.on("disconnect", (reason) => {
    if (findHatUser(socket.id)) {
      ...
      io.to(hatRef(findHatUser(socket.id).currentRoom)).emit(
        "users",
        getPrettyUsers(findHatUser(socket.id).currentRoom)
      );
    }
  });
});
